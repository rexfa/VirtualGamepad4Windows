...
namespace VirtualGamepad4Windows
{
    public class VGLightBar
    {
        ...
        public static VGColor[] forcedColor = new VGColor[4];
        ...
        public static VGColor HuetoRGB(float hue, byte sat)
        {
            byte C = sat;
            int X = (int)((C * (float)(1 - Math.Abs((hue / 60) % 2 - 1))));
            if (0 <= hue && hue < 60)
                return new VGColor(C, (byte)X, 0);
            else if (60 <= hue && hue < 120)
                return new VGColor((byte)X, C, 0);
            else if (120 <= hue && hue < 180)
                return new VGColor(0, C, (byte)X);
            else if (180 <= hue && hue < 240)
                return new VGColor(0, (byte)X, C);
            else if (240 <= hue && hue < 300)
                return new VGColor((byte)X, 0, C);
            else if (300 <= hue && hue < 360)
                return new VGColor(C, 0, (byte)X);
            else
                return new VGColor(Color.Red);
        }
    }
}
